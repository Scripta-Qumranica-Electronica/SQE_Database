...
char * translate(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error);
...
char *translate(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error) {
    ...
    *response = *length; // Store the size of the current polygon (we check it in later row to see if a resize is needed)
    ...
    int x_offset = *args->args[1];
    int y_offset = *args->args[2];
