...
CREATE TABLE `SQE_image` (
  ...
  PRIMARY KEY (`sqe_image_id`) USING BTREE,
  UNIQUE KEY `url_UNIQUE` (`image_urls_id`,`filename`) USING BTREE,
  KEY `fk_image_to_catalog` (`image_catalog_id`) USING BTREE,
  KEY `filename_idx` (`filename`) USING BTREE,
...
CREATE TABLE `SQE_image_author` (
  ...
  PRIMARY KEY (`sqe_image_id`) USING BTREE,
  KEY `SQE_image_owner_to_scroll_version_id` (`user_id`) USING BTREE,
  ...
  CONSTRAINT `fk_SQE_image_owner_to_sqe_image_id` FOREIGN KEY (`sqe_image_id`) REFERENCES `SQE_image` (`sqe_image_id`) ON DELETE CASCADE ON UPDATE NO ACTION
...
CREATE TABLE `SQE_image_settings` (
  ...
  CONSTRAINT `fk_sqe_image_settings_to_sqe_image` FOREIGN KEY (`SQE_image_id`) REFERENCES `SQE_image` (`sqe_image_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `SQE_image_settings_owner` (
  ...
  CONSTRAINT `fk_SQE_image_settings_owner_to_SQE_image_settings` FOREIGN KEY (`SQE_image_settings_id`) REFERENCES `SQE_image_settings` (`SQE_image_settings_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `area_group` (
  ...
  KEY `fk_area_group_to_creator_id` (`creator_id`),
  CONSTRAINT `fk_area_group_to_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `user` (`user_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `artefact_data` (
  ...
  KEY `fk_artefact_data_to_artefact` (`artefact_id`) USING BTREE,
  KEY `fk_artefact_data_to_creator_id` (`creator_id`),
  CONSTRAINT `fk_artefact_data_to_artefact` FOREIGN KEY (`artefact_id`) REFERENCES `artefact` (`artefact_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
) ENGINE=InnoDB AUTO_INCREMENT=48119 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='This stores metadata about the artefact.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `artefact_data_owner` (
  ...
  PRIMARY KEY (`artefact_data_id`,`edition_id`) USING BTREE,
  KEY `fk_artefact_data_owner_to_scroll_version_id` (`edition_editor_id`) USING BTREE,
  KEY `fk_artefact_data_to_edition` (`edition_id`) USING BTREE,
  CONSTRAINT `fk_artefact_data_owner_to_artefact_data_id` FOREIGN KEY (`artefact_data_id`) REFERENCES `artefact_data` (`artefact_data_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
) ENGINE=InnoDB AUTO_INCREMENT=33 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='This table holds the abstract IDs for the artefact group.  An artefact group consists of a name (see artefact_group_name) and a list of members (see artefact_group_member).  It is up to the user to determine what an artefact group is meant to do functionally.  Typically we assume that when one member of a group is transformed, all members of the group will also be transformed accordingly.  The responsibility for such operations, however, lies downstream from the database (i.e., there are no database triggers involved with artefact groups).';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `artefact_group_member_owner` (
  ...
  CONSTRAINT `artefact_group_member_owner_to_artefact_group_member` FOREIGN KEY (`artefact_group_member_id`) REFERENCES `artefact_group_member` (`artefact_group_member_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `artefact_image` (
  ...
  CONSTRAINT `fk_artefact_image_to_artefact_id` FOREIGN KEY (`artefact_id`) REFERENCES `artefact` (`artefact_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `artefact_image_owner` (
  ...
  CONSTRAINT `artefact_image_owner_to_artefact_image` FOREIGN KEY (`artefact_image_id`) REFERENCES `artefact_image` (`artefact_image_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `artefact_position` (
  ...
  `mirrored` tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT 'A boolean flag.  A value of 1 means that the artefact should appear mirrored.',
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`artefact_position_id`) USING BTREE,
  UNIQUE KEY `fk_unique_artefact_position` (`artefact_id`,`rotate`,`scale`,`translate_x_non_null`,`translate_y_non_null`,`z_index`,`mirrored`) USING BTREE,
  KEY `fk_artefact_position_to_artefact` (`artefact_id`) USING BTREE,
...
CREATE TABLE `artefact_position_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`artefact_position_id`,`edition_id`) USING BTREE,
  KEY `fk_artefact_position_owner_to_scroll_version` (`edition_editor_id`) USING BTREE,
  KEY `fk_artefact_position_to_edition` (`edition_id`) USING BTREE,
...
CREATE TABLE `artefact_shape` (
  ...
  `region_in_sqe_image` geometry NOT NULL COMMENT 'This is the exact polygon of the artefact’s location within the master image’s coordinate system, but alwaya at a resolution of 1215 PPI. If the master image is not 1215 PPI it should be scaled to that resolution before the artefact is drawn upon it.',
  ...
  CONSTRAINT `fk_artefact_shape_to_artefact` FOREIGN KEY (`artefact_id`) REFERENCES `artefact` (`artefact_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  ...
  CONSTRAINT `fk_artefact_shape_to_sqe_image` FOREIGN KEY (`sqe_image_id`) REFERENCES `SQE_image` (`sqe_image_id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=59305 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='This table holds the polygon describing the region of the artefact in the coordinate system of its image. The image must first be scaled to the PPI defined in manuscript_metrics (1215 PPI by default).';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `artefact_shape_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`artefact_shape_id`,`edition_id`) USING BTREE,
  KEY `fk_artefact_shape_owner_to_scroll_version` (`edition_editor_id`) USING BTREE,
  KEY `fk_artefact_shape_to_edition` (`edition_id`) USING BTREE,
  CONSTRAINT `fk_artefact_shape_owner_to_artefact_shape` FOREIGN KEY (`artefact_shape_id`) REFERENCES `artefact_shape` (`artefact_shape_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `artefact_stack` (
  ...
  KEY `fk_af_stack_A_to_artefact_idx` (`artefact_A_id`) USING BTREE,
  KEY `fk_af_stack_B_to_artefact_idx` (`artefact_B_id`) USING BTREE,
  KEY `fk_artefact_stack_to_creator_id` (`creator_id`),
  CONSTRAINT `fk_af_stack_A_to_artefact` FOREIGN KEY (`artefact_A_id`) REFERENCES `artefact` (`artefact_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `fk_af_stack_B_to_artefact` FOREIGN KEY (`artefact_B_id`) REFERENCES `artefact` (`artefact_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `artefact_stack_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`artefact_stack_id`,`edition_id`) USING BTREE,
  KEY `fk_artefact_data_owner_to_scroll_version_id` (`edition_editor_id`) USING BTREE,
  KEY `fk_artefact_stack_owner_to_artefact_stack_idx` (`artefact_stack_id`) USING BTREE,
  KEY `fk_artefact_stack_to_edition` (`edition_id`) USING BTREE,
  CONSTRAINT `fk_artefact_stack_owner_to_artefact_stack` FOREIGN KEY (`artefact_stack_id`) REFERENCES `artefact_stack` (`artefact_stack_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TABLE `artefact_status` (
  ...
  `artefact_id` int(11) unsigned NOT NULL,
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`artefact_status_id`) USING BTREE,
  UNIQUE KEY `unique_artefact_status` (`artefact_id`,`work_status_id`) USING BTREE,
  KEY `fk_artefact_status_to_work_status_id` (`work_status_id`) USING BTREE,
...
CREATE TABLE `artefact_status_owner` (
  ...
  `edition_id` int(11) unsigned NOT NULL,
  `edition_editor_id` int(11) unsigned NOT NULL,
  PRIMARY KEY (`artefact_status_id`,`edition_id`) USING BTREE,
  KEY `fk_artefact_status_owner_to_edition_id` (`edition_id`) USING BTREE,
  KEY `fk_artefact_status_owner_to_edition_editor_id` (`edition_editor_id`) USING BTREE,
...
--
-- Temporary table structure for view `artefact_view`
--

DROP TABLE IF EXISTS `artefact_view`;
...
CREATE TABLE `attribute` (
  ...
  `repeatable` tinyint(3) unsigned NOT NULL DEFAULT 1 COMMENT 'This flag functions as a boolean (0: false, 1: true) determining whether an attribute_value of this attribute type may be added more than once to the same sign interpretation.',
  `batch_editable` tinyint(3) unsigned NOT NULL DEFAULT 1 COMMENT 'This flag functions as a boolean (0: false, 1: true) to determine whether this attribute is intended to be set for multiple entities at the same time.',
...
CREATE TABLE `attribute_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`attribute_id`,`edition_id`) USING BTREE,
  KEY `fk_attribute_to_scroll_version` (`edition_editor_id`) USING BTREE,
  KEY `fk_attribute_to_edition` (`edition_id`) USING BTREE,
...
CREATE TABLE `attribute_value` (
  ...
  KEY `fk_att_val_to_att_idx` (`attribute_id`) USING BTREE,
...
CREATE TABLE `attribute_value_css` (
  ...
  KEY `fk_attribute_value_css_to_attribute_value` (`attribute_value_id`) USING BTREE,
...
CREATE TABLE `attribute_value_css_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`attribute_value_css_id`,`edition_id`) USING BTREE,
  KEY `fk_attribute_value_css_owner_to_scroll_version` (`edition_editor_id`) USING BTREE,
  KEY `fk_attribute_value_css_to_edition` (`edition_id`) USING BTREE,
...
CREATE TABLE `attribute_value_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`attribute_value_id`,`edition_id`) USING BTREE,
  KEY `fk_attribute_value_to_scroll_version` (`edition_editor_id`) USING BTREE,
  KEY `fk_attribute_value_to_edition` (`edition_id`) USING BTREE,
...
--
-- Table structure for table `dry_line_shape`
--

DROP TABLE IF EXISTS `dry_line_shape`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `dry_line_shape` (
  `dry_line_shape_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `artefact_id` int(10) unsigned NOT NULL,
  `line` linestring NOT NULL,
  `creator_id` int(10) unsigned NOT NULL,
  PRIMARY KEY (`dry_line_shape_id`),
  KEY `dry_line_shape_to_artefact___fk` (`artefact_id`),
...
--
-- Table structure for table `dry_line_shape_owner`
--

DROP TABLE IF EXISTS `dry_line_shape_owner`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `dry_line_shape_owner` (
  `dry_line_shape_id` int(10) unsigned NOT NULL,
  `edition_id` int(10) unsigned NOT NULL,
  `edition_editor_id` int(10) unsigned NOT NULL,
  UNIQUE KEY `dry_line_shape_owner_dry_line_shape_id_edition_id_uindex` (`dry_line_shape_id`,`edition_id`),
  KEY `dry_line_shape_to_edition___fk` (`edition_id`),
  KEY `dry_line_shape_to_edition_editor___fk` (`edition_editor_id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=2078 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='This table provides the anchor for a complete scholarly edition of a manuscript.  It also maintains the locked and public status of the edition.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `edition_editor` (
  ...
  PRIMARY KEY (`edition_editor_id`) USING BTREE,
  UNIQUE KEY `edition_user_idx` (`edition_id`,`user_id`) USING BTREE,
  KEY `fk_edition_editor_to_user` (`user_id`) USING BTREE,
...
) ENGINE=InnoDB AUTO_INCREMENT=2112 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Each edition has one or more edition editors, which are the individual users working on that edition.  Each edition editor has individual access rights that are specified here.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `edition_editor_request` (
  ...
  CONSTRAINT `edition_editor_request_to_admin_user_id` FOREIGN KEY (`admin_user_id`) REFERENCES `user` (`user_id`),
  CONSTRAINT `edition_editor_request_to_edition_id` FOREIGN KEY (`edition_id`) REFERENCES `edition` (`edition_id`),
  CONSTRAINT `edition_editor_request_to_editor_user_id` FOREIGN KEY (`editor_user_id`) REFERENCES `user` (`user_id`),
  CONSTRAINT `edition_editor_request_to_token` FOREIGN KEY (`token`) REFERENCES `user_email_token` (`token`)
...
--
-- Temporary table structure for view `edition_iaa_manifest`
--

DROP TABLE IF EXISTS `edition_iaa_manifest`;
...
) ENGINE=InnoDB AUTO_INCREMENT=40583 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='This table contains the IAA data for the editio princeps reference for all of their images.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `iaa_edition_catalog_author` (
  ...
  `user_id` int(11) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`iaa_edition_catalog_id`) USING BTREE,
  KEY `fk_edition_catalog_owner_to_scroll_version_id` (`user_id`) USING BTREE,
...
CREATE TABLE `iaa_edition_catalog_to_text_fragment` (
  ...
  `text_fragment_id` int(11) unsigned NOT NULL,
  PRIMARY KEY (`iaa_edition_catalog_to_text_fragment_id`) USING BTREE,
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 trigger SQE.prevent_impossible_insert_to_edition_catalog_to_col_confirmation
	before insert
	on SQE.iaa_edition_catalog_to_text_fragment_confirmation
	for each row
	BEGIN
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 trigger SQE.prevent_impossible_update_to_edition_catalog_to_col_confirmation
	before update
	on SQE.iaa_edition_catalog_to_text_fragment_confirmation
	for each row
	BEGIN
...
--
-- Table structure for table `iaa_image_metadata`
--

DROP TABLE IF EXISTS `iaa_image_metadata`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
...
CREATE TABLE `image_catalog` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`image_catalog_id`) USING BTREE,
  ...
  KEY `object-id-index` (`object_id`) USING BTREE,
...
) ENGINE=InnoDB AUTO_INCREMENT=49081 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='The referencing system of the institution providing the images.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `image_catalog_author` (
  ...
  `user_id` int(11) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`image_catalog_id`) USING BTREE,
  KEY `fk_image_catalog_owner_to_scroll_version_id` (`user_id`) USING BTREE,
...
--
-- Table structure for table `image_catalog_owner`
--

DROP TABLE IF EXISTS `image_catalog_owner`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `image_catalog_owner` (
  `image_catalog_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `edition_id` int(11) unsigned NOT NULL,
  `edition_editor_id` int(11) unsigned NOT NULL,
  PRIMARY KEY (`image_catalog_id`,`edition_id`),
  KEY `image_catalog_owner_to_edition` (`edition_id`),
  KEY `image_catalog_owner_to_edition_editor` (`edition_editor_id`),
...
CREATE TABLE `image_to_iaa_edition_catalog` (
  `iaa_edition_catalog_id` int(11) unsigned NOT NULL DEFAULT 0,
  `image_catalog_id` int(11) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`iaa_edition_catalog_id`,`image_catalog_id`) USING BTREE,
  KEY `fk_to_catalog_id` (`image_catalog_id`) USING BTREE,
...
CREATE TABLE `image_to_image_map` (
  ...
  `rotate` decimal(6,2) NOT NULL DEFAULT 0.00,
  `translate_x` int(10) NOT NULL DEFAULT 0,
  `translate_y` int(10) NOT NULL DEFAULT 0,
  ...
  PRIMARY KEY (`image_to_image_map_id`) USING BTREE,
  UNIQUE KEY `unique_image_to_image_map` (`image1_id`,`image2_id`,`region1_hash`,`region2_hash`,`scale`,`rotate`,`translate_x`,`translate_y`) USING BTREE,
  KEY `fk_image1_to_image_id` (`image1_id`) USING BTREE,
  KEY `fk_image2_to_image_id` (`image2_id`) USING BTREE,
  CONSTRAINT `fk_image1_to_image_id` FOREIGN KEY (`image1_id`) REFERENCES `SQE_image` (`sqe_image_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `fk_image2_to_image_id` FOREIGN KEY (`image2_id`) REFERENCES `SQE_image` (`sqe_image_id`) ON DELETE CASCADE ON UPDATE NO ACTION
...
CREATE TABLE `image_to_image_map_author` (
  ...
  `user_id` int(11) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`image_to_image_map_id`,`user_id`) USING BTREE,
  KEY `fk_image_to_image_map_author_to_image_to_user` (`user_id`) USING BTREE,
  CONSTRAINT `fk_image_to_image_map_author_to_image_to_image_map` FOREIGN KEY (`image_to_image_map_id`) REFERENCES `image_to_image_map` (`image_to_image_map_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
) ENGINE=InnoDB AUTO_INCREMENT=54476 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='The line is an abstract placeholder which can receive definition via the line_data table.  It must be nested in a text fragment (text_fragment_to_line) and will contain signs (line_to_sign)';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `line_data` (
  ...
  KEY `fk_line_data_to_line_idx` (`line_id`) USING BTREE,
...
CREATE TABLE `line_data_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`line_data_id`,`edition_id`) USING BTREE,
  KEY `fk_line_data_owner_to_scroll_version_idx` (`edition_editor_id`) USING BTREE,
  KEY `fk_line_data_to_edition` (`edition_id`) USING BTREE,
...
CREATE TABLE `line_to_sign` (
  ...
  `line_id` int(10) unsigned NOT NULL,
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`line_to_sign_id`) USING BTREE,
  UNIQUE KEY `line_sign_idx` (`sign_id`,`line_id`) USING BTREE,
  KEY `fk_line_to_sign_to_line_idx` (`line_id`) USING BTREE,
...
CREATE TABLE `line_to_sign_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`line_to_sign_id`,`edition_id`) USING BTREE,
  KEY `fl_to_sign_owner_to_scroll_version_idx` (`edition_editor_id`) USING BTREE,
  KEY `fk_line_to_sign_to_edition` (`edition_id`) USING BTREE,
...
CREATE TABLE `main_action` (
  ...
  PRIMARY KEY (`main_action_id`) USING BTREE,
  UNIQUE KEY `all_idx` (`main_action_id`,`edition_id`,`edition_editor_id`) USING BTREE,
  KEY `main_action_to_scroll_version_idx` (`edition_editor_id`) USING BTREE,
  KEY `fk_main_action_to_edition` (`edition_id`) USING BTREE,
...
) ENGINE=InnoDB AUTO_INCREMENT=1805 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Description of a reconstructed manuscript or combination.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `manuscript_data_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`manuscript_data_id`,`edition_id`) USING BTREE,
...
--
-- Temporary table structure for view `manuscript_text_fragment_line_sign_view`
--

DROP TABLE IF EXISTS `manuscript_text_fragment_line_sign_view`;
...
CREATE TABLE `manuscript_to_text_fragment` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`manuscript_to_text_fragment_id`) USING BTREE,
...
CREATE TABLE `manuscript_to_text_fragment_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`manuscript_to_text_fragment_id`,`edition_id`) USING BTREE,
...
--
-- Temporary table structure for view `manuscript_view`
--

DROP TABLE IF EXISTS `manuscript_view`;
...
CREATE TABLE `parallel_group` (
  ...
  PRIMARY KEY (`parallel_group_id`) USING BTREE
...
CREATE TABLE `parallel_group_pair` (
  ...
  KEY `fk_parallel_group_pair_to_creator_id` (`creator_id`),
  ...
  CONSTRAINT `fk_parallel_group_pair_to_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `user` (`user_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Creates a pair of parallel_groups';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `parallel_group_pair_to_type` (
  ...
  KEY `fk_parallel_group_pair_to_type_to_creator_id` (`creator_id`),
  ...
  CONSTRAINT `fk_parallel_group_pair_to_type_to_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `user` (`user_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Connexts a parallel_group_pair with a parallel type';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `parallel_sign_stream_section` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`parallel_sign_stream_section_id`) USING BTREE,
  ...
  KEY `fk_parallel_sign_stream_section_to_creator_id` (`creator_id`),
  ...
  CONSTRAINT `fk_parallel_sign_stream_section_to_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `user` (`user_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `parallel_sign_stream_section_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`parallel_sign_stream_section_id`,`edition_id`) USING BTREE,
...
CREATE TABLE `parallel_type` (
  ...
  KEY `fk_parallel_type_to_creator_id` (`creator_id`),
  CONSTRAINT `fk_parallel_type_to_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `user` (`user_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_parallel_type_to_parent` FOREIGN KEY (`parent_type_id`) REFERENCES `parallel_type` (`parallel_type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Hierarchical list which defines parallel_types';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `point_to_point_map` (
  ...
  PRIMARY KEY (`point_to_point_map_id`) USING BTREE,
  KEY `fK_p_to_p_toIm_to_im_idx` (`image_to_image_map_id`) USING BTREE,
...
CREATE TABLE `position_in_stream` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`position_in_stream_id`) USING BTREE,
  KEY `fk_position_in_stream_to_sign_interpretation` (`sign_interpretation_id`) USING BTREE,
  KEY `fk_position_in_stream_to_next_sign_interpretation` (`next_sign_interpretation_id`) USING BTREE,
...
CREATE TABLE `position_in_stream_owner` (
  ...
  PRIMARY KEY (`position_in_stream_id`,`edition_id`) USING BTREE,
  KEY `fk_position_in_stream_owner_to_edition_id` (`edition_id`) USING BTREE,
  KEY `fk_position_in_stream_owner_to_edition_editor_id` (`edition_editor_id`) USING BTREE,
...
CREATE TABLE `position_in_stream_to_section_rel` (
  ...
  `sign_stream_section_id` int(10) unsigned NOT NULL COMMENT 'Reference to sign_stream_section',
  PRIMARY KEY (`position_in_stream_id`,`sign_stream_section_id`) USING BTREE,
...
CREATE TABLE `position_in_text_fragment_stream` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`position_in_text_fragment_stream_id`) USING BTREE,
  KEY `fk_pitfs_next_to_text_fragment` (`next_text_fragment_id`) USING BTREE,
  KEY `fk_pitfs_to_text_fragment` (`text_fragment_id`) USING BTREE,
...
CREATE TABLE `position_in_text_fragment_stream_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL,
  `is_main` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'If true, the next item will be\nfirst in the list of next items. There should\nonly one next item marked this way\nfor each item of this owner.',
  PRIMARY KEY (`position_in_text_fragment_stream_id`,`edition_id`) USING BTREE,
  KEY `fk_pitfs_owner_to_edition` (`edition_id`) USING BTREE,
  KEY `fk_pitfs_owner_to_editor` (`edition_editor_id`) USING BTREE,
...
CREATE TABLE `qwb_variant` (
  ...
  KEY `fk_qwb_var_to_qwb_biblio_idx` (`qwb_biblio_id`) USING BTREE,
...
CREATE TABLE `roi_position` (
  ...
  PRIMARY KEY (`roi_position_id`) USING BTREE,
  KEY `fk_roi_position_to_artefact` (`artefact_id`) USING BTREE,
  CONSTRAINT `fk_roi_position_to_artefact` FOREIGN KEY (`artefact_id`) REFERENCES `artefact` (`artefact_id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=24086 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ROI’s are linked to artefacts.  To get the location of the ROI in the coordinate system of the “virtual manuscript, one must first apply the roi_position and then the position of the linked artefact. This can by done via the UDF nested_geom_transform.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `roi_shape` (
  ...
  PRIMARY KEY (`roi_shape_id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=20545 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='This table holds the polygon describing the ROI in its own coordinate system. The roi_position table situates the polygon in the coordinate system of the artefact.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `scribal_font_kerning` (
  ...
  UNIQUE KEY `unique_scribal_font_kerning` (`scribal_font_id`,`first_unicode_char`,`second_unicode_char`,`kerning_x`,`kerning_y`) USING BTREE,
...
CREATE TABLE `scribal_font_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`scribal_font_id`,`edition_id`) USING BTREE,
  KEY `fk_scribal_font_type_to_edition` (`edition_id`) USING BTREE,
...
CREATE TABLE `scribe_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`scribe_id`,`edition_id`) USING BTREE,
  KEY `fk_scribe_owner_to_scroll_version_idx` (`edition_editor_id`) USING BTREE,
  KEY `fk_scribe_to_edition` (`edition_id`) USING BTREE,
...
) ENGINE=InnoDB AUTO_INCREMENT=1734891 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='This is an abstract placeholder allowing a multiplicity of interpretations to be related to each other by linking to the same sign_id.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sign_interpretation` (
  ...
  PRIMARY KEY (`sign_interpretation_id`) USING BTREE,
...
CREATE TABLE `sign_interpretation_attribute` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`sign_interpretation_attribute_id`) USING BTREE,
  UNIQUE KEY `unique_sign_interpretation_attribute_sequence` (`sign_interpretation_id`,`attribute_value_id`,`sequence`),
...
CREATE TABLE `sign_interpretation_attribute_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`sign_interpretation_attribute_id`,`edition_id`) USING BTREE,
...
--
-- Table structure for table `sign_interpretation_character`
--

DROP TABLE IF EXISTS `sign_interpretation_character`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
...
--
-- Table structure for table `sign_interpretation_character_owner`
--

DROP TABLE IF EXISTS `sign_interpretation_character_owner`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `sign_interpretation_character_owner` (
  ...
  KEY `fk_sichar_owner_to_edition_editor` (`edition_editor_id`),
  CONSTRAINT `fk_sichar_owner_to_edition_editor` FOREIGN KEY (`edition_editor_id`) REFERENCES `edition_editor` (`edition_editor_id`),
...
CREATE TABLE `sign_interpretation_commentary_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`sign_interpretation_commentary_id`,`edition_id`) USING BTREE,
...
CREATE TABLE `sign_interpretation_roi` (
  ...
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`sign_interpretation_roi_id`) USING BTREE,
  UNIQUE KEY `unique_sign_interpretation_shape_position` (`sign_interpretation_id`,`roi_shape_id`,`roi_position_id`) USING BTREE,
  KEY `fk_sign_area_to_area_idx` (`roi_shape_id`) USING BTREE,
  KEY `fk_sign_area_to_area_position_idx` (`roi_position_id`) USING BTREE,
...
CREATE TABLE `sign_interpretation_roi_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`sign_interpretation_roi_id`,`edition_id`) USING BTREE,
  KEY `fk_sign_area_owner_to_sv_idx` (`edition_editor_id`) USING BTREE,
...
--
-- Temporary table structure for view `sign_interpretation_view`
--

DROP TABLE IF EXISTS `sign_interpretation_view`;
...
) ENGINE=OQGRAPH DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci `data_table`='position_in_stream' `origid`='sign_interpretation_id' `destid`='next_sign_interpretation_id';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sign_stream_reverse` (
  ...
  KEY `latch` (`latch`,`origid`,`destid`) USING HASH,
  KEY `latch_2` (`latch`,`destid`,`origid`) USING HASH
) ENGINE=OQGRAPH DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci `data_table`='position_in_stream' `origid`='next_sign_interpretation_id' `destid`='sign_interpretation_id';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sign_stream_section_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`sign_stream_section_id`,`edition_id`) USING BTREE,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Provides n:m connection between qwb words and sections of the sign stream.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `single_action` (
  ...
  KEY `single_action_table_idx` (`table`) USING BTREE,
...
--
-- Temporary table structure for view `sqe_image_iaa_metadata`
--

DROP TABLE IF EXISTS `sqe_image_iaa_metadata`;
...
--
-- Table structure for table `sqe_image_to_iaa_image_metadata`
--

DROP TABLE IF EXISTS `sqe_image_to_iaa_image_metadata`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `sqe_image_to_iaa_image_metadata` (
  `sqe_image_id` int(11) unsigned NOT NULL,
  `iaa_image_metadata_id` int(11) unsigned NOT NULL,
  PRIMARY KEY (`sqe_image_id`,`iaa_image_metadata_id`),
...
CREATE TABLE `text_fragment` (
  ...
  PRIMARY KEY (`text_fragment_id`) USING BTREE
...
CREATE TABLE `text_fragment_data_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`text_fragment_data_id`,`edition_id`) USING BTREE,
...
CREATE TABLE `text_fragment_to_line` (
  ...
  `line_id` int(10) unsigned NOT NULL,
  `creator_id` int(11) unsigned NOT NULL DEFAULT 1,
  PRIMARY KEY (`text_fragment_to_line_id`) USING BTREE,
...
CREATE TABLE `text_fragment_to_line_owner` (
  ...
  `edition_editor_id` int(10) unsigned NOT NULL DEFAULT 0,
  `edition_id` int(10) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`text_fragment_to_line_id`,`edition_id`) USING BTREE,
...
--
-- Temporary table structure for view `text_fragment_view`
--

DROP TABLE IF EXISTS `text_fragment_view`;
...
CREATE TABLE `user_email_token` (
  ...
  KEY `fk_user_email_token_to_user` (`user_id`) USING BTREE,
...
--
-- Table structure for table `virtual_column_shape`
--

DROP TABLE IF EXISTS `virtual_column_shape`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `virtual_column_shape` (
  `virtual_column_shape_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
--
-- Table structure for table `virtual_column_shape_owner`
--

DROP TABLE IF EXISTS `virtual_column_shape_owner`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `virtual_column_shape_owner` (
  `virtual_column_shape_id` int(10) unsigned NOT NULL,
  `edition_id` int(10) unsigned NOT NULL,
  `edition_editor_id` int(10) unsigned NOT NULL,
  UNIQUE KEY `virtual_column_shape_owner__uindex` (`virtual_column_shape_id`,`edition_id`),
  KEY `virtual_column_shape_owner_to_edition__fk` (`edition_id`),
  KEY `virtual_column_shape_owner_to_edition_editor__fk` (`edition_editor_id`),
...
/*!50106 CREATE*/ /*!50117 DEFINER=`root`@`localhost`*/ /*!50106 EVENT `clear_old_email_tokens` ON SCHEDULE EVERY 1 HOUR STARTS '2019-05-21 16:05:40' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM user_email_token
WHERE user_id IN (
    SELECT user_id
...
/*!50001 VIEW `edition_iaa_manifest` AS select `SQE_image`.`sqe_image_id` AS `sqe_image_id`,`image_catalog_owner`.`edition_id` AS `edition_id`,`iaa_image_metadata`.`iaa_image_metadata_id` AS `iaa_image_metadata_id`,`iaa_image_metadata`.`iaa_file_id` AS `old_iaa_filename`,`SQE_image`.`image_urls_id` AS `image_urls_id`,`SQE_image`.`filename` AS `filename`,`SQE_image`.`native_width` AS `native_width`,`SQE_image`.`native_height` AS `native_height`,`SQE_image`.`dpi` AS `dpi`,`SQE_image`.`type` AS `type`,`SQE_image`.`wavelength_start` AS `wavelength_start`,`SQE_image`.`wavelength_end` AS `wavelength_end`,`SQE_image`.`is_master` AS `is_master`,`SQE_image`.`image_catalog_id` AS `image_catalog_id`,`SQE_image`.`is_recto` AS `is_recto`,`SQE_image`.`manifest` AS `manifest`,`iaa_image_metadata`.`material` AS `material`,`iaa_image_metadata`.`publication_number` AS `publication_number`,`iaa_image_metadata`.`plate` AS `plate`,`iaa_image_metadata`.`frag` AS `frag`,`iaa_image_metadata`.`site` AS `site`,`iaa_image_metadata`.`period` AS `period`,`iaa_image_metadata`.`composition` AS `composition`,`iaa_image_metadata`.`copy` AS `copy`,`iaa_image_metadata`.`manuscript` AS `manuscript`,`iaa_image_metadata`.`other_identifications` AS `other_identifications`,`iaa_image_metadata`.`abbreviation` AS `abbreviation`,`iaa_image_metadata`.`manuscript_type` AS `manuscript_type`,`iaa_image_metadata`.`composition_type` AS `composition_type`,`iaa_image_metadata`.`language` AS `language`,`iaa_image_metadata`.`script` AS `script`,`iaa_image_metadata`.`publication` AS `publication`,`iaa_image_metadata`.`date_imaged` AS `date_imaged`,`iaa_image_metadata`.`photographer` AS `photographer`,`iaa_image_metadata`.`image_type` AS `image_type`,`iaa_image_metadata`.`row` AS `row`,`iaa_image_metadata`.`column` AS `column` from (((`SQE_image` join `image_catalog_owner` on(`SQE_image`.`image_catalog_id` = `image_catalog_owner`.`image_catalog_id`)) join `sqe_image_to_iaa_image_metadata` on(`SQE_image`.`sqe_image_id` = `sqe_image_to_iaa_image_metadata`.`sqe_image_id`)) join `iaa_image_metadata` on(`sqe_image_to_iaa_image_metadata`.`iaa_image_metadata_id` = `iaa_image_metadata`.`iaa_image_metadata_id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`%` SQL SECURITY INVOKER */
/*!50001 VIEW `sqe_image_iaa_metadata` AS select `SQE_image`.`sqe_image_id` AS `sqe_image_id`,`iaa_image_metadata`.`iaa_image_metadata_id` AS `iaa_image_metadata_id`,`iaa_image_metadata`.`iaa_file_id` AS `old_iaa_filename`,`SQE_image`.`image_urls_id` AS `image_urls_id`,`SQE_image`.`filename` AS `filename`,`SQE_image`.`native_width` AS `native_width`,`SQE_image`.`native_height` AS `native_height`,`SQE_image`.`dpi` AS `dpi`,`SQE_image`.`type` AS `type`,`SQE_image`.`wavelength_start` AS `wavelength_start`,`SQE_image`.`wavelength_end` AS `wavelength_end`,`SQE_image`.`is_master` AS `is_master`,`SQE_image`.`image_catalog_id` AS `image_catalog_id`,`SQE_image`.`is_recto` AS `is_recto`,`SQE_image`.`manifest` AS `manifest`,`iaa_image_metadata`.`material` AS `material`,`iaa_image_metadata`.`publication_number` AS `publication_number`,`iaa_image_metadata`.`plate` AS `plate`,`iaa_image_metadata`.`frag` AS `frag`,`iaa_image_metadata`.`site` AS `site`,`iaa_image_metadata`.`period` AS `period`,`iaa_image_metadata`.`composition` AS `composition`,`iaa_image_metadata`.`copy` AS `copy`,`iaa_image_metadata`.`manuscript` AS `manuscript`,`iaa_image_metadata`.`other_identifications` AS `other_identifications`,`iaa_image_metadata`.`abbreviation` AS `abbreviation`,`iaa_image_metadata`.`manuscript_type` AS `manuscript_type`,`iaa_image_metadata`.`composition_type` AS `composition_type`,`iaa_image_metadata`.`language` AS `language`,`iaa_image_metadata`.`script` AS `script`,`iaa_image_metadata`.`publication` AS `publication`,`iaa_image_metadata`.`date_imaged` AS `date_imaged`,`iaa_image_metadata`.`photographer` AS `photographer`,`iaa_image_metadata`.`image_type` AS `image_type`,`iaa_image_metadata`.`row` AS `row`,`iaa_image_metadata`.`column` AS `column`,`iaa_image_metadata`.`width` AS `width`,`iaa_image_metadata`.`height` AS `height` from ((`SQE_image` join `sqe_image_to_iaa_image_metadata` on(`SQE_image`.`sqe_image_id` = `sqe_image_to_iaa_image_metadata`.`sqe_image_id`)) join `iaa_image_metadata` on(`sqe_image_to_iaa_image_metadata`.`iaa_image_metadata_id` = `iaa_image_metadata`.`iaa_image_metadata_id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
